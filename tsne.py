...
def pca(X = np.array([]), initial_dims = 50):
    ...
    S = np.dot(centered_X.transpose(), centered_X)

    eig_values, eig_vectors = np.linalg.eig(S)

    sorted_w_idx = eig_values.argsort()[::-1][:initial_dims]
    
    U = eig_vectors[:, sorted_w_idx]
    
    Y = np.dot(centered_X, U)
...
def compute_gradient(P, Q, Y_sim, Y, dY, no_dims):
    ...
    L_dash = np.diag(np.sum(L, axis=0))
    dY = 4 * np.dot(L_dash - L, Y)
    #print dY
    return dY
